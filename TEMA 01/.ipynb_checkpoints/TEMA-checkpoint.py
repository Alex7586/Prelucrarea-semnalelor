...
# Yjpeg, y, y_jpeg = jpg(Y)
# y_nnz = np.count_nonzero(y)
# y_jpeg_nnz = np.count_nonzero(y_jpeg)
# print('Pentru Y\': \nComponente în frecvență:' + str(y_nnz) + 
#       '\nComponente în frecvență după cuantizare: ' + str(y_jpeg_nnz))

# Cbjpeg, y, y_jpeg = jpg(Cb)
# y_nnz = np.count_nonzero(y)
# y_jpeg_nnz = np.count_nonzero(y_jpeg)
# print('Pentru Cb: \nComponente în frecvență:' + str(y_nnz) + 
#       '\nComponente în frecvență după cuantizare: ' + str(y_jpeg_nnz))

# Crjpeg, y, y_jpeg = jpg(Cr)
# y_nnz = np.count_nonzero(y)
# y_jpeg_nnz = np.count_nonzero(y_jpeg)
# print('Pentru Cr: \nComponente în frecvență:' + str(y_nnz) + 
#       '\nComponente în frecvență după cuantizare: ' + str(y_jpeg_nnz))
...
# imgJPEG = RGB_transform(img_ycbcr_jpeg)


# plt.subplot(121).imshow(img, cmap=plt.cm.gray)
# plt.title('Original')
# plt.subplot(122).imshow(imgJPEG, cmap=plt.cm.gray)
# plt.title('JPEG')
# plt.show()

# mse_target = 1000
# X = datasets.ascent()
# x_jpeg, y, y_jpeg = jpg_until_mse(X, Q_jpeg, mse_target)
# plt.subplot(121).imshow(X, cmap=plt.cm.gray)
# plt.title('Original')
# plt.subplot(122).imshow(x_jpeg, cmap=plt.cm.gray)
# plt.title('JPEG')
...
video_path = SCRIPT_DIR / "Facultate" / "AN 3" / "Procesarea semnalelor" / "Prelucrarea-semnalelor" / "TEMA 01" / "filmuletz.mp4"
...
if not writer.isOpened():
    print("Error: Could not open VideoWriter. (încearcă XVID + .avi)")
    sys.exit(1)
